...
function groupAdminFamily(persons, relationships) {
  ...
  const relevantLinks = (relationships || []).filter(
    rel => Number(rel.person_id) === selfId || Number(rel.related_person_id) === selfId
  );
  ...
  relevantLinks.forEach((rel) => {
    ...
    const fromSelf = Number(rel.person_id) === selfId;
    const otherPersonId = fromSelf ? Number(rel.related_person_id) : Number(rel.person_id);
    const related = personById.get(otherPersonId);
