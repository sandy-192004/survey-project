...
function connectorPath(parent, child) {
  const startX = parent.left * NODE_WIDTH + NODE_WIDTH / 2;
  const startY = parent.top * NODE_HEIGHT + NODE_HEIGHT;
  const endX = child.left * NODE_WIDTH + NODE_WIDTH / 2;
  const endY = child.top * NODE_HEIGHT;
...
export default function FamilyTree({ initialRootId }) {
  ...
  const loadTree = React.useCallback(async (personId) => {
    ...
    try {
      ...
      const normalized = normalizeRelativesTree(data.relativesTree);
      const calculated = calcTree(normalized, Number(data.rootId));
  ...
  const canvas = React.useMemo(() => {
    ...
    return {
      width: Math.max((maxLeft + 2.5) * NODE_WIDTH, 1600),
      height: Math.max((maxTop + 2.5) * NODE_HEIGHT, 900)
    };
  }, [drawNodes]);
